  ...
  if (currentUrl === newTabPageUrl) {
    ...
    const draw = () => {
      ...
      if (analyser && isPlaying) {
        ...
        dataArray = dataArray || new Uint8Array(analyser ? analyser.frequencyBinCount : 0);
        dataArray.fill(0);
      }
    ...
    const populateSongSelect = (currentSessionHandles = []) => {
      ...
      playlist = [...defaultSettings.defaultPlaylistUrls, ...currentSessionHandles.map(item => item.handle)];
